...
class Movielens_Learner(QObject, BaseEstimator):
    ...
    def __init__(
        ...
        self.parar_entrenamiento = (
            False  # flag que se usa para interrumpir un entrenamiento
        )
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(), tf.device("/cpu:0"):
            ...
            # datos de entrada
            self.user = tf.compat.v1.placeholder(tf.int32, shape=None, name="Usuario")
            self.best_movie = tf.compat.v1.placeholder(
                tf.int32, shape=None, name="Pelicula_1"
            )
            self.worst_movie = tf.compat.v1.placeholder(
                tf.int32, shape=None, name="Pelicula_2"
            )
            ...
            self.W = tf.Variable(
                tf.random.truncated_normal(
                    (self.K, self.num_users), stddev=1.0 / np.sqrt(self.K)
                ),
                name="Usuarios_Embedding_W",
            )
            self.V = tf.Variable(
                tf.random.truncated_normal(
                    (self.K, self.num_movies), stddev=1.0 / np.sqrt(self.K)
                ),
                name="Peliculas_Embedding_V",
            )

            user_embedding = tf.gather(self.W, self.user, axis=1, name="usuario_emb")
            best_movie_embedding = tf.gather(
                self.V, self.best_movie, axis=1, name="pelicula_1_emb"
            )
            worst_movie_embedding = tf.gather(
                self.V, self.worst_movie, axis=1, name="pelicula_2_emb"
            )

            self.__user_score = self.F_embeddings(
                user_embedding, best_movie_embedding
            ...
            self.loss = (
                tf.reduce_mean(input_tensor=preference_loss_with_margin)
                + self.tf_nu * reg
            )
            ...
            if self.optimizer == "Adam":
                ...
                self.opt = tf.compat.v1.train.AdagradOptimizer(
                    learning_rate=self.tf_learning_rate
                ).minimize(loss=self.loss)
            else:  # SGD
    ...
    def F(self, user, movie):
        ...
        result = tf.linalg.tensor_diag_part(tf.matmul(f_aux, movie, transpose_b=True))
        return result
    ...
    def F_embeddings(self, user_embedding, movie_embedding):
        ...
        # matricial
        result = tf.linalg.tensor_diag_part(
            tf.matmul(user_embedding, movie_embedding, transpose_a=True)
        )
        return result
    ...
    def fit(self, data, test_data):
        ...
        session.run(
            [
                self.tf_learning_rate.assign(self.learning_rate),
        ...
        for epoch in range(self.num_epochs):
            for step in range(steps_per_epoch):
                ...
                if (t2 - t1) >= self.drawevery or ultima_iteracion:
                    ...
                    if (
                        len(data_interactive_user) > 0
                    ):  # pero sólo si ha dado algunas puntuaciones
                        feed_dict = {
                            self.user: data_interactive_user.user,
    ...
    def predict(self, data):
        ...
        test_set = generate_next_sequential_batch(
            data, num_examples, reset_last=True
        )  # Todos los datos secuencialmente
    ...
    def save(self, path):
        ...
        json.dump(
            params, open(os.path.join(path, "model_params.json"), "w", encoding="utf8")
        )
    ...
    def restore_on_created_object(self, path):
        ...
        params = json.load(
            open(os.path.join(path_dir, "model_params.json"), "r", encoding="utf8")
        )
